...
test -f "${source_dir}introduction.html"
...
ssh -o BatchMode=yes -o ConnectTimeout=15 "$AETHERARK_REMOTE_HOST" \
  "test -f '$AETHERARK_REMOTE_DIR/index.html' && test -f '$AETHERARK_REMOTE_DIR/introduction.html' && test -f '$AETHERARK_REMOTE_DIR/data.html' && test -f '$AETHERARK_REMOTE_DIR/modeling.html' && test -f '$AETHERARK_REMOTE_DIR/training-results.html'"
